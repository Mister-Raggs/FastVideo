"""NVFP4 linear quantization inference for Cosmos-Predict2.5 2B (Blackwell).

Runs Cosmos-2.5 2B t2w with NVFP4-quantized DiT linear layers (attention
to_q/k/v/out projections and FFN mlp.fc_in/fc_out) via the flashinfer-backed
``nvfp4_qat`` config. Weights are quantized once at load and the dense bf16
copies are dropped, roughly halving DiT weight memory.

Linear-only: attention math stays in the default backend. (FP4 *attention*
is a separate opt-in backend — see the sm_121a ATTN_QAT_INFER work.)
...
Requirements:
    - Blackwell GPU (sm100+; RTX 5090, DGX Spark GB10)
    - flashinfer for the NVFP4 quantize/matmul ops

Usage:
    python nvfp4_qat_cosmos2_5_2b.py            # NVFP4 linear
    python nvfp4_qat_cosmos2_5_2b.py --bf16     # BF16 baseline
"""
...
def main():
    parser = argparse.ArgumentParser(description="NVFP4 Cosmos-2.5 2B generation benchmark")
    ...
    parser.add_argument("--seed", type=int, default=1024,
                        help="Generation seed; keep fixed across bf16/nvfp4 arms for SSIM comparison")
    args = parser.parse_args()

    from fastvideo import VideoGenerator
    from fastvideo.layers.quantization import get_quantization_config

    mode = "bf16" if args.bf16 else "nvfp4_qat"
    if args.compile:
        mode += "_compile"
    ...
    extra = {} if args.bf16 else {"transformer_quant": get_quantization_config("nvfp4_qat")()}
