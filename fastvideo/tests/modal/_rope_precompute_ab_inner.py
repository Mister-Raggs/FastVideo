"""
Inner-script half of the W3a Wan rotary-precompute A/B harness.

Runs one denoising pass over the harness's prompt + seed set, records
per-prompt walls, and writes a JSON results blob to the path given on
argv. No runtime flag — the A/B is between two GIT REFS, not two
runtime configurations, so this script always runs with whatever code
is currently installed in /opt/venv. The outer harness reinstalls
between passes.

Invoked by ``rope_precompute_ab.py`` via ``/opt/venv/bin/python`` so
...
def main() -> int:
    ...
    print(f"[inner] label={label}", flush=True)
