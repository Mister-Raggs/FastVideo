"""
Inner-script half of the W4 DBCache A/B harness.

Runs one denoising pass (DBCache off or on) over the harness's prompt +
seed set, records per-prompt walls, and writes a JSON results blob to the
path given on argv.

Invoked by ``dbcache_ab.py`` via ``/opt/venv/bin/python`` so that
...
Unlike the W3 harnesses, DBCache is LOSSY: the patched output is not
bit-identical, so the SSIM column is a quality measurement (target
>= 0.95), not a 1.0 correctness gate.
"""
import argparse
import json
import os
...
def main() -> int:
    ...
    use_dbcache: bool = cfg["use_dbcache"]
    dbcache_fn_compute_blocks: int = cfg.get("dbcache_fn_compute_blocks", 8)
    ...
    for i, prompt in enumerate(prompts):
        ...
        # pick the one we just wrote by mtime (newest).
        mp4s = sorted(
        ...
        label = "dbcache" if use_dbcache else "baseline"
        print(f"[inner] [{label}] prompt {i}: {wall:.3f}s -> {mp4s[-1]}", flush=True)
...
def _compute_ssim_main(args) -> int:
    ...
    to initialise. SSIM runs on CPU, so we inline the logic here using
    only torch / torchvision / av directly.
    """
    ...
    # so our numbers are directly comparable to the canonical helper.
    import torch
