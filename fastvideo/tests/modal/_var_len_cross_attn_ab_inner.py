"""
Inner-script half of the W3i var-len cross-attn A/B harness.

Runs one denoising pass (unmasked baseline or var-len patched) over the
harness's prompt + seed set, records per-prompt walls, and writes a
JSON results blob to the path given on argv.

Invoked by ``var_len_cross_attn_ab.py`` via ``/opt/venv/bin/python`` so
that FastVideo runs inside the image's venv (Modal's main function
process runs in Modal's own Python, where FastVideo is not installed).
"""
import argparse
import json
import os
...
def main() -> int:
    ...
    use_var_len_cross_attn: bool = cfg["use_var_len_cross_attn"]
    ...
    resolved_flag = getattr(generator.fastvideo_args, "use_var_len_cross_attn", None)
    if resolved_flag is not use_var_len_cross_attn:
        raise RuntimeError(f"use_var_len_cross_attn did not propagate: requested "
                           f"{use_var_len_cross_attn}, resolved {resolved_flag}")
    print(f"[inner] use_var_len_cross_attn resolved to {resolved_flag}", flush=True)
    ...
    for i, prompt in enumerate(prompts):
        ...
        label = "var_len" if use_var_len_cross_attn else "unmasked"
