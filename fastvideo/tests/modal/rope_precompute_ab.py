"""
Ref-vs-ref A/B harness for the Wan rotary-precompute hoist (W3a).

Single-container pattern: install baseline_ref, run N seed-pinned
prompts, then re-install patched_ref into the same container and run
the same N prompts again. Records per-prompt walls + pairwise SSIM,
prints a delta table.

Unlike ``rope_precompute_ab.py`` (which toggles a runtime flag between
passes), this harness toggles which git ref is installed. The inner
script subprocess is launched fresh per pass so each side picks up
whatever fastvideo is currently in /opt/venv. Same GPU, same model
load timing, same prompts/seeds — variance bounded to the ref delta.

Modeled on the overlay-pattern Kuan used for #1395
(``nccl_stream_ab.py``) — same container, isolates the one-change
under test from container/topology variance.

Usage (from FastVideo repo root):

    modal run --detach fastvideo/tests/modal/rope_precompute_ab.py \\
        --gpu L40S --model wan2_1-t2v-1.3b --num-gpus 1 \\
        --baseline-ref main --patched-ref perf/wan-rope-precompute-v2
...
app = modal.App("wan-rope-precompute-ab")
...
def _run_pass(*, label: str, model_id: str, num_gpus: int, height: int, width: int, num_frames: int,
    ...
    runs inside the image's venv. ``label`` is a free-form tag (e.g.
    ``baseline``/``patched``) used only for log readability.

    The inner script picks up whatever fastvideo is currently installed
    in the venv — so swap installs between calls to A/B two git refs.
    """
    import json
    import subprocess
    import tempfile
    ...
    inner_script = "/FastVideo/fastvideo/tests/modal/_rope_precompute_ab_inner.py"
...
def _print_table(rows: list[dict]) -> None:
    print("\n=== wan-rope-precompute A/B results (baseline=main, patched=PR) ===")
...
def run_ab(
    *,
    ...
    """Ref-vs-ref A/B inside one container. Installs baseline_ref, runs
    one pass, then re-installs patched_ref and runs the second pass. Same
    container/model_load/GPU between passes — variance bounded to git-ref
    delta. The inner subprocess starts fresh per pass so the freshly
    installed fastvideo is picked up on each side.
    """
    import subprocess
    ...
    def _install(ref: str, *, first_install: bool) -> None:
        # On the first install we let the workspace builder do the full
        # clone + kernel build + HF login. On the second install only
        # the ref changes — the FA3 wheel, fastvideo-kernel build, and
        # HF login are already in place, so a checkout + reinstall is
        # all we need. ``_build_workspace_command`` handles both via
        # the existing-dir branch (no re-clone, fetch + checkout only).
        result = subprocess.run(
            ["/bin/bash", "-lc",
             _build_workspace_command(git_repo, ref,
                                      install_fa3=(install_fa3 and first_install))],
            env=os.environ.copy(),
            capture_output=True,
            text=True,
        )
        if result.returncode != 0:
            print(result.stdout)
            print(result.stderr)
            raise RuntimeError(f"workspace setup failed for ref={ref} (rc={result.returncode})")
    ...
    baseline_dir = f"/root/data/ab_out_rope/{model_preset}/{mode_tag}/baseline"
    patched_dir = f"/root/data/ab_out_rope/{model_preset}/{mode_tag}/patched"

    print(f"\n--- installing baseline ref: {baseline_ref} ---")
    _install(baseline_ref, first_install=True)
    os.chdir("/FastVideo")
    print(f"\n--- baseline pass (ref={baseline_ref}, mode={mode_tag}) on {model_preset} ---")
    baseline = _run_pass(label="baseline", output_dir=baseline_dir, **common)

    print(f"\n--- reinstalling patched ref: {patched_ref} ---")
    _install(patched_ref, first_install=False)
    os.chdir("/FastVideo")
    print(f"\n--- patched pass (ref={patched_ref}, mode={mode_tag}) on {model_preset} ---")
    patched = _run_pass(label="patched", output_dir=patched_dir, **common)
...
def probe_fa_version() -> str:
    ...
    Run: modal run fastvideo/tests/modal/rope_precompute_ab.py::probe_fa_version
    """
    import subprocess
    import sys
...
print('Run main A/B (which installs both refs in sequence)')
...
def recover_ssim(
    *,
    ...
    Run: modal run fastvideo/tests/modal/rope_precompute_ab.py::recover_ssim --git-repo ... --git-ref ...
    """
    import json
    import subprocess
    import tempfile
    ...
    cmd = (f"source /opt/venv/bin/activate && exec python "
           f"/FastVideo/fastvideo/tests/modal/_rope_precompute_ab_inner.py --mode compute_ssim "
           f"--baseline-results-json {baseline_path} --patched-results-json {patched_path} "
           f"--ssim-output-json {ssim_path}")
    subprocess.run(["/bin/bash", "-lc", cmd], check=True)
...
def main(
    ...
    patched_ref: str = "perf/wan-rope-precompute-v2",
    enable_compile: bool = False,
    num_prompts: int = 0,
):
    """Drive the ref-vs-ref A/B from your laptop. ``gpu`` is the Modal
    GPU class (``L40S``, ``H100``, ...). ``num_gpus=0`` uses the model
    preset default. ``git_repo`` defaults to the ``fork`` remote (where
    PR branches live) and falls back to ``origin``."""
    import subprocess
    ...
    print(f"GPU: {gpu}:{num_gpus}  model: {model}  baseline={baseline_ref}  patched={patched_ref}  "
          f"install_fa3: {install_fa3}  enable_compile: {enable_compile}  "
          f"num_prompts: {num_prompts or 'all'}  repo: {git_repo}")
