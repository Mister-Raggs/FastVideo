"""
A/B harness for ``use_var_len_cross_attn`` on Wan cross-attention.

Single-container pattern: clone the patched branch, build once, then
run the same N seed-pinned prompts twice — once with
``use_var_len_cross_attn=False`` (legacy unmasked cross-attn over the
full padded T5 sequence) and once with ``use_var_len_cross_attn=True``
(mask wired through to the FA backend's varlen path so only the real
text positions get attended). Records per-prompt wall + pairwise
SSIM/LPIPS, prints a delta table.

Output is NOT bit-equivalent across the two passes: the patched run
corrects the softmax dilution from T5 padding positions. The harness
measures the magnitude of that change (SSIM) and whether the resulting
video is still visually coherent. Combine with a separate diffusers-
reference comparison if you need to validate direction-of-correctness.

Modeled on ``batched_cfg_ab.py`` (W3b) — same overlay-pattern Kuan used
for #1395 (``nccl_stream_ab.py``), same container/model load, isolates
the one-flag change from container/topology variance.

Usage (from FastVideo repo root):

    modal run fastvideo/tests/modal/var_len_cross_attn_ab.py --gpu L40S \\
        --model wan2_1-t2v-1.3b --num-gpus 1

    modal run fastvideo/tests/modal/var_len_cross_attn_ab.py --gpu H100 \\
...
app = modal.App("var-len-cross-attn-ab")
...
def _run_pass(*, use_var_len_cross_attn: bool, model_id: str, num_gpus: int, height: int, width: int, num_frames: int,
              num_inference_steps: int, output_dir: str, enable_compile: bool,
              num_prompts: int | None = None) -> list[dict]:
    """Invoke the inner pass script (``_var_len_cross_attn_ab_inner.py``) via
    ...
    config = {
        "model_id": model_id,
        "num_gpus": num_gpus,
        "use_var_len_cross_attn": use_var_len_cross_attn,
    ...
    inner_script = "/FastVideo/fastvideo/tests/modal/_var_len_cross_attn_ab_inner.py"
...
def _print_table(rows: list[dict]) -> None:
    print("\n=== var_len_cross_attn A/B results ===")
...
def run_ab(
    *,
    ...
    print(f"\n--- baseline pass (use_var_len_cross_attn=False, mode={mode_tag}) on {model_preset} ---")
    baseline = _run_pass(use_var_len_cross_attn=False, output_dir=baseline_dir, **common)

    print(f"\n--- patched pass (use_var_len_cross_attn=True, mode={mode_tag}) on {model_preset} ---")
    patched = _run_pass(use_var_len_cross_attn=True, output_dir=patched_dir, **common)
...
def probe_fa_version() -> str:
    ...
    Run: modal run fastvideo/tests/modal/var_len_cross_attn_ab.py::probe_fa_version
    """
    import subprocess
    import sys
...
print('Run main A/B (which does git checkout of perf/wan-var-len-cross-attn)')
...
def recover_ssim(
    *,
    ...
    Run: modal run fastvideo/tests/modal/var_len_cross_attn_ab.py::recover_ssim --git-repo ... --git-ref ...
    """
    import json
    import subprocess
    import tempfile
    ...
    cmd = (f"source /opt/venv/bin/activate && exec python "
           f"/FastVideo/fastvideo/tests/modal/_var_len_cross_attn_ab_inner.py --mode compute_ssim "
           f"--baseline-results-json {baseline_path} --patched-results-json {patched_path} "
           f"--ssim-output-json {ssim_path}")
    subprocess.run(["/bin/bash", "-lc", cmd], check=True)
...
def main(
    ...
    num_gpus: int = 0,
    git_repo: str = "",
    git_ref: str = "perf/wan-var-len-cross-attn",
    enable_compile: bool = False,
    num_prompts: int = 0,
):
