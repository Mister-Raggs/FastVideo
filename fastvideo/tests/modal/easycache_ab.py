"""
A/B harness for EasyCache (model-agnostic, training-free adaptive step caching)
on the Wan DiT.
...
``enable_easycache=True`` at a given ``--thresh``. Records per-prompt wall +
pairwise SSIM and prints a delta table. Because EasyCache is model-*agnostic*
(it operates on the whole-forward residual, not per-model blocks), the SAME
harness validates any model — run it on a second ``--model`` to prove that.

EasyCache caching is LOSSY: the SSIM column measures the quality cost (target
>= ~0.95), not a 1.0 correctness gate. The "win" is the wall delta. Sweep
``--thresh`` (higher = more skipped steps = faster / lower quality). The denoise
log also prints an ``EasyCache: ...% steps skipped`` summary.

Usage (from FastVideo repo root):

    # Wan 1.3B, EasyCache at the default threshold, L40S
    modal run fastvideo/tests/modal/easycache_ab.py --gpu L40S

    # More aggressive (recover more wall): higher threshold
    modal run fastvideo/tests/modal/easycache_ab.py --gpu L40S --thresh 0.15

    # Prove model-agnosticism: same harness, different model
    modal run fastvideo/tests/modal/easycache_ab.py --gpu L40S --model wan2_2-t2v-14b
...
def _run_pass(*, enable_easycache: bool, model_id: str, num_gpus: int, preset: dict, output_dir: str,
    ...
    inner = "/FastVideo/fastvideo/tests/modal/_easycache_ab_inner.py"
...
def _print_table(rows: list[dict], label: str) -> None:
    print(f"\n=== EasyCache A/B results ({label}) ===")
    print("NOTE: EasyCache is lossy — SSIM is a quality cost (target >= ~0.95), not a 1.0 gate.")
    ...
    print(f"easycache total: {patched_sum:.3f}s")
...
def run_ab(*, git_repo: str, git_ref: str, model_preset: str, num_gpus: int, install_fa3: bool = False,
    ...
    print(f"\n--- EasyCache pass ({label}) ---")
    patched = _run_pass(enable_easycache=True, output_dir=patched_dir, **common)
